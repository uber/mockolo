...
struct Resolver {
    ...
    static func lookupEntities(key: String,
        ...
        if var current = protocolMap[key] {
            
            if  let curModels = current.models {
                models.append(contentsOf: curModels)
            }
            if let curAttributes = current.attributes {
            ...
            if let parentModels = parentMock.models {
                processedModels.append(contentsOf: parentModels)
            }
            if let parentAttributes = parentMock.attributes {
                attributes.append(contentsOf: parentAttributes)
            }
    ...
    static private func uniquifyDuplicates(group: [String: [Model]],
                                           level: Int,
                                           lookup: [String: Model]?,
    ...
    static func uniqueEntities(`in` models: [Model], exclude: [String: Model], fullnames: [String]) -> [String: Model] {
    ...
    static func potentialInitVars(`in` models: [String: Model], processed: [String: Model]) -> [VariableModel]? {
    ...
    static func findImportLines(content: String) -> [String] {
